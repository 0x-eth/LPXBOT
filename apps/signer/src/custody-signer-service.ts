...
export class CustodySignerService implements WalletDirectory, WalletSignerClient {
  ...
  readonly #localHelperUpgradePlanAuthorizer: LocalHelperUpgradePlanAuthorizer | null;
  ...
  constructor(input: {
    ...
    this.#localHelperUpgradePlanAuthorizer = input.localHelperUpgradePlanAuthorizer ?? null;
  ...
  localHelperUpgradeSigningConfigured(): boolean {
    return this.#localHelperUpgradePlanAuthorizer !== null && this.#rawTransactionDelivery !== null;
  }
  ...
  async signLocalHelperUpgrade(input: {
    generation: number;
    maxFeePerGasBaseUnit: string;
    maxPriorityFeePerGasBaseUnit: string;
    operationId: string;
    ...
    const authorizer = this.#localHelperUpgradePlanAuthorizer;
    const delivery = this.#rawTransactionDelivery;
    if (!authorizer || !delivery) throw new SignerError("SIGNER_UNAVAILABLE", true);
    const authorization = {
      generation: input.generation,
    ...
    // The second authorization closes the database/chain TOCTOU window before key use.
    if (!(await authorizer.authorize(authorization))) {
      throw new SignerError("LOCAL_HELPER_UPGRADE_PLAN_REJECTED");
    }
    return this.#signer.signAndDeliverLocalHelperUpgrade({
      delivery,
      envelope,
      generation: input.generation,
      maxFeePerGasBaseUnit: input.maxFeePerGasBaseUnit,
