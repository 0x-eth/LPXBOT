  ...
  return (
    <section aria-labelledby="custom-rpc-title" className="custom-rpc-settings">
      <div className="interface-section-heading">
        <div><Server aria-hidden="true" size={18} /><h2 id="custom-rpc-title">自定义只读 RPC</h2></div>
        <span className="rpc-state" data-state={state} role="status">
          {state === "testing" ? <LoaderCircle aria-hidden="true" className="spin-icon" size={15} /> : state === "ready" ? <CheckCircle2 aria-hidden="true" size={15} /> : <WifiOff aria-hidden="true" size={15} />}
          {stateLabels[state]}
        </span>
      </div>
      <form className="custom-rpc-panel" onSubmit={(event) => void test(event)}>
        <label>
          <span>链</span>
          <select aria-label="自定义 RPC 链" onChange={(event) => setChainId(Number(event.target.value))} value={chainId}>
            {chains.map((value) => <option key={value} value={value}>{chainNames[value]}</option>)}
          </select>
        </label>
        <label className="rpc-url-field">
          ...
          <input
            ...
            autoCapitalize="off"
            autoComplete="off"
            onBlur={() => { if (rawUrl !== "" && redacted !== "<invalid>") setEditing(false); }}
            onChange={(event) => { setRawUrl(event.target.value); setState("unconfigured"); }}
        ...
        <div className="custom-rpc-actions">
          <button className="secondary-button" disabled={state === "testing" || rawUrl === ""} type="submit">
            {state === "testing" ? <LoaderCircle aria-hidden="true" className="spin-icon" size={16} /> : <Server aria-hidden="true" size={16} />}
            测试
          </button>
          <button aria-label="清除自定义 RPC" className="icon-button danger-button" disabled={rawUrl === ""} onClick={clear} title="清除" type="button"><Trash2 aria-hidden="true" size={16} /></button>
        </div>
        {blockNumber ? <p className="rpc-block-number">区块 {blockNumber}</p> : null}
        {errorCode ? <p className="custom-rpc-error" role="alert">{errorCode}</p> : null}
      </form>
    </section>
  );
}
