...
export function usePoolCreatorLookup(input: {
  ...
  useEffect(() => {
    ...
    if (!input.enabled || keys.length === 0) {
      setStored({ requestKey, state: emptyCreatorLookup("ready") });
      return;
    }
    ...
    setStored({ requestKey, state: emptyCreatorLookup("loading") });

    // React StrictMode cleans up the first development-only effect before this timer fires.
