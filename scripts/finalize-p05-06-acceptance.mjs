...
const FROZEN = ["P05-03", "P05-04", "P05-05"];
const REQUIRED = [
  "E-API.md",
  "E-CHAIN.md",
  "E-DATA.md",
...
if (JSON.stringify(files) !== JSON.stringify(REQUIRED)) {
  ...
  throw new Error(
    `P05-06 acceptance inventory mismatch; missing=${missing.join(",")}; unexpected=${unexpected.join(",")}`,
  );
}
...
if (
  ...
  manifest.workItemId !== "P05-06" ||
  manifest.status !== "accepted-with-gaps"
) {
  throw new Error("P05-06 execution contract, gate, or manifest invariant failed");
}
...
console.log(
  `Finalized P05-06 acceptance: ${files.length} checksummed files; ${FROZEN.join(", ")} match ${BASELINE}.`,
);
