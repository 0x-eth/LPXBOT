...
async function main() {
  ...
  console.log(
    `Finalized P03-01 reference artifacts: ${files.length} files, ${fixtureCount} fixtures, ${priorCount} frozen prior-acceptance files, ${manifest.references.length} references.`,
  );
}

main().catch((error) => {
  console.error(`P03-01 reference artifact finalization failed: ${error.message}`);
  process.exitCode = 1;
});
