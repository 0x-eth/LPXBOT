...
contract WalletHelperV2 {
    using SafeERC20 for IERC20;

    bool public immutable ATOMIC_LIQUIDITY_EXECUTION_ENABLED;
    ...
    error AtomicLiquidityExecutionClosed();
    ...
    constructor(
        ...
        adapter = adapter_;
        permit2 = permit2_;
        ATOMIC_LIQUIDITY_EXECUTION_ENABLED = false;
    ...
    function executeAtomicLiquidity(bytes32, AtomicLiquidityPlan calldata, Permit2Authorization calldata)
        external
        onlyOwner
        nonReentrant
        returns (uint256, uint256, uint256)
    {
        if (!ATOMIC_LIQUIDITY_EXECUTION_ENABLED) revert AtomicLiquidityExecutionClosed();
        return (0, 0, 0);
    }
