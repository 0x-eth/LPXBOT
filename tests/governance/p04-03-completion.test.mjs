...
test("P04 status is exactly 5 implemented-assumed / 7 planned with global 54 / 142", async () => {
  ...
  assert.equal([...rows].filter(([, row]) => row.status === "planned").length, 7);
  ...
  assert.match(traceability, /\| `implemented-assumed` \| 54 \|/u);
  assert.match(traceability, /\| 其余 `planned` \| 142 \|/u);
...
test("P00 through P04-02 acceptance files are byte-identical to the requested baseline", () => {
  const changed = execFileSync(
    ...
    .split("\n")
    .filter(Boolean)
    .filter((file) => !file.startsWith("artifacts/acceptance/P04-03/"));
