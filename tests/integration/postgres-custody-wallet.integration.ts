...
const migration = readFileSync(
  path.resolve("infra/migrations/20260818000100_create_custody_wallets.sql"),
  "utf8",
);
...
beforeAll(async () => {
  ...
  await pool.query(
    `INSERT INTO users (id, role, tier, status, display_name)
     VALUES ($1, 'user', 'normal', 'active', 'A'), ($2, 'user', 'normal', 'active', 'B')`,
    [userA, userB],
  );
  await pool.query(up!);
});
...
describe("P04-02 PostgreSQL custody wallet store", () => {
  it("runs migration down/up and preserves constraints", async () => {
    ...
    try {
      ...
      expect((await client.query("SELECT to_regclass('custody_wallet_envelopes') AS table_name")).rows[0].table_name).toBe(
  ...
  it("rolls back wallet, envelope, pointer, and audit after a transaction fault", async () => {
    ...
    for (const table of ["custody_wallets", "custody_wallet_envelopes", "custody_wallet_audit_events"]) {
      expect((await pool.query(`SELECT count(*)::int AS count FROM ${table} WHERE wallet_id = $1`, [walletId])).rows[0].count).toBe(0);
    }
  });
});
