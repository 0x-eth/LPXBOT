...
const walletTransferMigration = readFileSync(
  path.resolve("infra/migrations/20260818000700_create_wallet_transfers.sql"),
  "utf8",
);
...
const [, walletHelperDown] = walletHelperMigration.split("-- migrate:down");
const pricingPositionMigration = readFileSync(
  path.resolve("infra/migrations/20260819000300_create_swap_quotes_pricing_positions.sql"),
  "utf8",
);
const [, pricingPositionDown] = pricingPositionMigration.split("-- migrate:down");
const helperDeploymentMigration = readFileSync(
  path.resolve("infra/migrations/20260819000400_create_helper_deployment_operations.sql"),
  "utf8",
);
const [, helperDeploymentDown] = helperDeploymentMigration.split("-- migrate:down");
const localSwapExecutionMigration = readFileSync(
  path.resolve("infra/migrations/20260820000100_create_local_swap_execution.sql"),
  "utf8",
);
const [, localSwapExecutionDown] = localSwapExecutionMigration.split("-- migrate:down");
const localPositionExecutionMigration = readFileSync(
  path.resolve("infra/migrations/20260820000200_create_local_position_execution.sql"),
  "utf8",
);
...
const localHelperUpgradeMigration = readFileSync(
  path.resolve("infra/migrations/20260821000100_create_local_helper_upgrade.sql"),
  "utf8",
);
const [, localHelperUpgradeDown] = localHelperUpgradeMigration.split("-- migrate:down");
...
describe("P04-02 PostgreSQL custody wallet store", () => {
  it("runs migration down/up and preserves constraints", async () => {
    ...
    try {
      ...
      await client.query(localHelperUpgradeDown!);
      ...
      await client.query(pricingPositionDown!);
