...
describe("P05-09 local Helper upgrade API domain", () => {
  ...
  it("previews version/steps and creates one plan-bound operation idempotently", async () => {
    ...
    expect(Object.keys(stored!.plan.target.tokenA).sort()).toEqual(["address", "runtimeCodeHash"]);
    expect(Object.keys(stored!.plan.target.tokenB).sort()).toEqual(["address", "runtimeCodeHash"]);
