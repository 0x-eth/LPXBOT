...
**当前状态：** P05-06 在 P05-04 local execution safety baseline 和 P05-05 per-wallet `WalletHelperV1` active binding 上新增独立 `p05-local-swap-execution-v2` Registry、`p05-local-swap-quote-v2` 与 `p05-local-swap-plan-v2`，将本地 quote、精确 approval/Permit2、隔离签名、逐 step 广播、canonical receipt、余额/minOut/Helper 事件/allowance/dust 核对、replacement、失败清理与重启恢复接成闭环。客户端只提交 wallet/quote/authorization/opaque preview 字段；每个 step 独立绑定 nonce、fencing token、calldata digest 和费用上限，replacement 只能提高费用；approve 已确认而 Swap 失败时必须完成 allowance=0 cleanup，未确认期间保持 `reconciling`。local gate 仅对非 fork Anvil chainId 31337、合成钱包、TestOnlyERC20/WBNB 与 SWAP-02 保持 `OPEN`；BSC quote 仍为 `executionEnabled=false`，testnet/production gates 均为 `CLOSED`，测试网/生产签名和广播及真实资金操作均为 0。P05 当前为 7 项 `implemented-assumed`、5 项 `planned`，继续 `accepted-with-gaps`；全局为 68 项 `implemented-assumed`、128 项 `planned`。POS-02、POS-03、HELPER-03、HELPER-04、HELPER-06 继续 planned；权威生产 Router/Helper 证据、独立合约审查与 testnet 证据仍 unresolved，不标记 `parity-verified` 或 `released`。
