...
**当前状态：** P05-08 在 P05-05 WalletHelperV1 binding 与 P05-06 nonce/fencing、隔离 Signer、replacement、canonical receipt 和恢复模式上新增独立 `p05-local-helper-sweep-v2` Registry、`p05-local-helper-residual-snapshot-v2` 和 `p05-local-helper-sweep-plan-v2`，将 native、TestOnlyERC20 与 WBNB 的残留扫描、逐资产 preview/签名/广播、receipt/Transfer、gas-adjusted 余额核对、replacement 和完整 canonical rescan 接成闭环。客户端只提交 wallet/chain/assets/snapshot 与 opaque preview 字段；每个资产独立绑定 nonce、fencing token、semantic/data digest、deadline 和 gas cap，replacement 只能加费。confirmed 资产不重放，失败或 dropped 资产从独立游标恢复；allowance、NFT custody 或 unknown Token 进入 manual recovery，binding 只有完整复扫全通过才恢复 active。local gate 仅对非 fork Anvil chainId 31337、合成钱包、P05-05 WalletHelperV1、allowlisted TestOnly 资产与 HELPER-06 保持 `OPEN`；chainId 56 继续只读，BSC/testnet/production gates 均为 `CLOSED`，非本地签名、广播和真实资金操作均为 0。P05 当前为 10 项 `implemented-assumed`、2 项 `planned`，继续 `accepted-with-gaps`；全局为 71 项 `implemented-assumed`、125 项 `planned`。HELPER-03、HELPER-04 继续 planned；权威生产 Helper/Router 证据、独立合约审查与 testnet 证据仍 unresolved，不标记 `parity-verified` 或 `released`。
