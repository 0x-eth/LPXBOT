...
| MON-04 | Telegram 与外部 Webhook | UI+API+BUNDLE | USER | R1 | GET/POST、模板占位符、测试、不保存测试配置；`implemented-assumed`（P03-03/P03-04，注入式本地投递 fixture；live delivery unresolved） |
| MON-05 | 通知历史 | UI+API | USER | R0 | 当前用户分页/筛选、稳定状态、monitor/destination 删除后保留快照；`implemented-assumed`（P03-04） |
| MON-06 | 投递安全 | INFERRED | USER | R2 | 独立 Dispatcher、SSRF/私网阻断、签名、TLS/超时、重试、租约和分层速率门；`implemented-assumed`（P03-04，注入式 fixture；live delivery 与 SLO unresolved） |
...
| ID | 功能 | 证据 | 权限 | 风险 | 复现与验收要点 |
|---|---|---|---|---|---|
| WALLET-01 | 导入私钥钱包 | UI+API | USER | R2 | 64 hex 校验；前端不回显；日志永不记录私钥；`implemented-assumed`（P04-02，local fixture only） |
| WALLET-02 | 服务端生成钱包 | UI+BUNDLE | USER | R2 | CSPRNG、地址回显、密文落库、备份策略；`implemented-assumed`（P04-02，local fixture only） |
| WALLET-03 | 命名、删除和强制删除 | UI+API | USER | R2/R3 | 关联策略/任务检查；资产/仓位风险说明；`implemented-assumed`（P04-04，local fixture only） |
...
| HELPER-02 | 首次使用自动部署 Helper | UI+BUNDLE+CHAIN | USER | R3/R4 | owner EOA 直接部署是当前 BSC 样本事实；部署幂等；`planned`（P05-04 仅建立 local deployment baseline） |
...
| NOTIFY-02 | 外部 Webhook 偏好 | UI+BUNDLE | USER | R1/R2 | GET/POST、body 模板、测试、密钥化签名；`implemented-assumed`（P03-03/P03-04，完整 SSRF egress 仅本地 fixture 验证；live delivery unresolved） |
