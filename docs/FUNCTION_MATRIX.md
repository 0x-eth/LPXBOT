...
| POOL-08 | 地址/Token 搜索 | UI+API | USER | R0 | 池地址、token 关联池；清除和无结果状态 |
| POOL-09 | 同 token 池折叠与分组 | UI+BUNDLE | USER | R0 | 组内展开、排序、`+N` 标记 |
| POOL-10 | 列显隐、拖动排序与重置 | UI+BUNDLE | USER | R1 | Pool/操作锁定，偏好跨设备同步 |
...
| STATS-02 | 推荐池 SSE | API+BUNDLE | USER | R0 | 约 5s、浅 hash 不变不推、链和 limit |
