...
> 当前状态：P01 的 18 项、P02 的 23 项、P03 的 8 项、P04 的 12 项及 P05 的 11 项功能已完成阶段实现，因目标对照和 live 证据缺口均保持 `implemented-assumed`；其余 124 项保持 `planned`。表中测试和证据是达到完成定义的最低要求。
...
| POOL-15 | P02 | T-UNIT,T-API,T-REC,T-MIG,T-UI,T-VIS,T-SEC | E-API,E-DATA,E-REC,E-UI,E-VIS,E-RBAC,E-SEC |
...
P02-02、P02-04、P02-05、P02-06、P02-07、P02-08、P02-09、P02-10、P02-11、P02-12 与 P02-13 只验证本地 fixture 纵向路径。P02 的 23 项均为 `implemented-assumed`，0 项为 `planned`，阶段功能实现至此收官；没有项目达到 `parity-verified` 或 `released`，P02 继续 `accepted-with-gaps`。P02-01 继续是无实现所有权的冻结参考契约，aTVL、Fee/aTVL、`GAP-LABEL-ALGORITHM`、`GAP-FLOW-USD-VALUATION`、`GAP-API-CANDLE-QUOTE`、`GAP-UI-TICK-LIQUIDITY-MAPPING`、既有 USD/公式缺口与 `GAP-FINALITY-DEPTH` 继续 unresolved。STATS-01 的任务业务域尚未接线；投影未 ready 时 GET/SSE 必须返回 503，不以零任务冒充权威来源。
...
| POOL-15 | `implemented-assumed` | [Frozen contract and internal recorder](../apps/api/src/pool-creation-provenance.ts), [PostgreSQL ledger](../apps/api/src/postgres-pool-creation-provenance-store.ts), [Migration](../infra/migrations/20260817000300_create_pool_creation_provenance.sql), [Read-only API and RBAC](../apps/api/src/app.ts), [Strict client](../apps/web/src/pool-provenance-client.ts), [History and attribution UI](../apps/web/src/pool-provenance-ui.tsx) | [T-UNIT](../tests/pool-creation-provenance.test.ts), [T-API/T-SEC](../tests/pool-creation-provenance-api.test.ts), [T-REC/T-MIG](../tests/integration/postgres-pool-creation-provenance.integration.ts), [T-UI/T-VIS](../tests/e2e/p02-12-pool-provenance.spec.ts) | [P02-12](../artifacts/acceptance/P02-12/manifest.json); local-fixture-verified; platform-recorded operation user only; no chain-origin inference; internal recorder only |
...
| STATS-01 | `implemented-assumed` | [Wire contract](../packages/api-contract/src/index.ts), [Authoritative publisher and PostgreSQL provider](../apps/api/src/shell-stats.ts), [Transactional migration](../infra/migrations/20260817000400_create_task_status_stats_projection.sql), [GET/SSE and RBAC](../apps/api/src/app.ts), [Strict client](../apps/web/src/shell-stats.ts), [Status bar](../apps/web/src/shell-stats-react.tsx) | [T-UNIT](../tests/task-status-stats.test.ts), [T-API/T-SSE/T-SEC](../tests/stats-sse-api.test.ts), [T-REC/T-MIG](../tests/integration/postgres-shell-stats.integration.ts), [T-UI](../tests/shell-stats-client.test.ts), [T-UI/T-VIS](../tests/e2e/p02-13-shell-stats.spec.ts) | [P02-13](../artifacts/acceptance/P02-13/manifest.json); local-fixture-verified; authoritative absolute counts; task business source not connected; unready returns 503 |
...
P03-02 至 P03-04 仅验证 BSC、local-sink 和注入式 DNS/HTTP/TLS/Telegram fixture 纵向路径。P03 功能实现已收官，为 8 项 `implemented-assumed`、0 项 `planned`，阶段保持 `accepted-with-gaps`。active TVL、Fee/aTVL、`GAP-P03-TARGET-MONITOR-PARITY`、live Telegram/Webhook delivery 与 delivery SLO 保持 unresolved；生产 history retention 仍是未决运维策略。非 monitor 分类当前只保存偏好，不生成尚不存在的业务事件；没有项目达到 `parity-verified` 或 `released`。
...
| MON-04 | `implemented-assumed` | [API contract](../packages/api-contract/src/index.ts), [HTTP API](../apps/api/src/app.ts), [PostgreSQL configuration store](../apps/api/src/postgres-notification-store.ts), [Destination selector](../apps/worker/src/postgres-monitor-destination-selector.ts), [Dispatcher](../apps/dispatcher/src/dispatcher.ts), [Webhook adapter](../apps/dispatcher/src/webhook-adapter.ts), [Telegram adapter](../apps/dispatcher/src/telegram-adapter.ts), [Notification UI](../apps/web/src/notification-settings.tsx) | [T-API/T-SEC](../tests/notification-settings-api.test.ts), [T-UNIT/T-SEC](../tests/notification-delivery-security.test.ts), [T-REC/T-MIG](../tests/integration/postgres-notifications.integration.ts), [T-UNIT/T-SEC](../tests/notification-dispatcher.test.ts), [T-UI/T-VIS](../tests/e2e/p03-03-notifications.spec.ts) | [P03-03](../artifacts/acceptance/P03-03/manifest.json), [P03-04](../artifacts/acceptance/P03-04/manifest.json); local-fixture-verified; live delivery unresolved |
| MON-05 | `implemented-assumed` | [History contract](../packages/api-contract/src/index.ts), [History API](../apps/api/src/notification-history.ts), [PostgreSQL history store](../apps/api/src/postgres-notification-history-store.ts), [Atomic Outbox projection](../apps/worker/src/postgres-monitor-outbox.ts), [Migration](../infra/migrations/20260818000200_create_notification_delivery_history.sql), [History UI](../apps/web/src/monitors-page.tsx) | [T-API](../tests/notification-history-api.test.ts), [T-REC/T-MIG](../tests/integration/postgres-notification-history.integration.ts), [T-UI/T-VIS](../tests/e2e/p03-04-notification-history.spec.ts) | [P03-04](../artifacts/acceptance/P03-04/manifest.json); local-fixture-verified; no automatic retention policy |
| MON-06 | `implemented-assumed` | [Dispatcher](../apps/dispatcher/src/dispatcher.ts), [Production boundary](../apps/dispatcher/src/production.ts), [Destination revision resolver](../apps/dispatcher/src/postgres-dispatch-destination-store.ts), [Webhook egress](../apps/dispatcher/src/webhook-adapter.ts), [Telegram adapter](../apps/dispatcher/src/telegram-adapter.ts), [Secret boundary](../apps/api/src/notifications.ts) | [T-UNIT/T-REC](../tests/notification-dispatcher.test.ts), [T-SEC](../tests/webhook-egress.test.ts), [T-SEC](../tests/telegram-delivery-adapter.test.ts), [T-REC/T-MIG](../tests/integration/postgres-notification-history.integration.ts) | [P03-04](../artifacts/acceptance/P03-04/manifest.json); local-fixture-verified with zero real notification network calls; live delivery and delivery SLO unresolved |
| NOTIFY-01 | `implemented-assumed` | [Preference contract](../packages/api-contract/src/index.ts), [HTTP API](../apps/api/src/app.ts), [PostgreSQL configuration store](../apps/api/src/postgres-notification-store.ts), [Destination selector](../apps/worker/src/postgres-monitor-destination-selector.ts), [Settings UI](../apps/web/src/notification-settings.tsx) | [T-API](../tests/notification-settings-api.test.ts), [T-REC](../tests/integration/postgres-notifications.integration.ts), [T-UI/T-VIS](../tests/e2e/p03-03-notifications.spec.ts) | [P03-03](../artifacts/acceptance/P03-03/manifest.json); local-fixture-verified; categories default off; only monitor-match has a business producer |
...
| WALLET-07 | `implemented-assumed` | [Independent Argon2id domain](../apps/signer/src/security-password-crypto.ts), [security-password lifecycle](../apps/signer/src/custody-signer-service.ts), [dedicated API ingress](../apps/api/src/app.ts), [internal signer port](../apps/signer/src/http-server.ts), [transfer ingress](../apps/api/src/wallet-transfers.ts), [settings UI](../apps/web/src/security-password-settings.tsx) | [T-UNIT/T-SEC](../tests/security-password.test.ts), [T-API/T-SEC](../tests/security-password-api.test.ts), [T-REC/T-MIG](../tests/integration/postgres-security-password.integration.ts), [T-API/T-SEC](../tests/wallet-transfer-api.test.ts), [T-UI/T-VIS](../tests/e2e/p04-06-wallet-transfer.spec.ts) | [P04-04](../artifacts/acceptance/P04-04/manifest.json), [P04-06](../artifacts/acceptance/P04-06/manifest.json); local-fixture-verified; new external transfer password is signer-verified and never persisted |
| WALLET-08 | `implemented-assumed` | [Wallet asset service and controlled provider port](../apps/api/src/wallet-assets.ts), [PostgreSQL token store](../apps/api/src/postgres-wallet-token-store.ts), [strict web client](../apps/web/src/wallet-read-client.ts), [asset and receive UI](../apps/web/src/wallet-read-panels.tsx), [migration](../infra/migrations/20260818000600_create_wallet_assets_address_book.sql) | [T-UNIT/T-CHAIN](../tests/wallet-assets.test.ts), [T-API](../tests/wallet-assets-api.test.ts), [T-REC/T-MIG](../tests/integration/postgres-wallet-assets-address-book.integration.ts), [T-UI](../tests/wallet-read-client.test.ts), [T-UI/T-VIS](../tests/e2e/p04-05-wallet-assets.spec.ts) | [P04-05](../artifacts/acceptance/P04-05/manifest.json); local-fixture-verified; exact strings and controlled provider; public RPC calls 0 |
...
P05-09 在 P05-05 WalletHelperV1 binding、P05-06 nonce/fencing 与隔离 Signer、P05-08 sweep/rescan 上新增 WalletHelperV2、独立 `p05-local-helper-upgrade-v3` Registry、snapshot v3、typed deploy-new plan 和七段持久化游标。preflight 阻断 degraded binding、live operation、provider divergence、owner/runtime/Registry drift 与 nonce 冲突；部署后验证 owner、runtime、ABI/selectors、adapter、Permit2 与 Token identities，再清场 V1 并在 SERIALIZABLE 数据库事务中 compare-and-swap 为 V1 superseded/V2 active。confirmed 部署和 sweep 重启不重放，allowance/NFT custody/unknown Token 转入 `manual-recovery-required` 且不生成任意 calldata；replacement 仅可加费。客户端仅提交 wallet/chain 与 opaque preview 字段，不能提交 bytecode/helper/target/selector/calldata/recipient/Registry/fee override。local gate 只对非 fork Anvil chainId 31337、合成钱包和 allowlisted TestOnly 组件的 HELPER-03 为 `OPEN`；WalletHelperV2 的 typed atomic liquidity ABI 已冻结但执行 gate 保持 `CLOSED`，HELPER-04 继续 planned，BSC/testnet/production signatures 与 broadcasts 均为 0。P05 当前为 11 项 `implemented-assumed`、1 项 `planned`，全局为 72 项 `implemented-assumed`、124 项 `planned`，工作项保持 `accepted-with-gaps`，不标记 `parity-verified` 或 `released`。
...
| POS-02 | `implemented-assumed` | [local Position Registry](../packages/chain-registry/src/local-position-execution.ts), [snapshot/plan domain](../packages/domain/src/local-position-execution.ts), [API service](../apps/api/src/local-position-executions.ts), [PostgreSQL ledger](../apps/api/src/postgres-local-position-execution-store.ts), [Signer authorizer](../apps/signer/src/postgres-local-position-plan-authorizer.ts), [Worker recovery](../apps/worker/src/postgres-local-position-recovery.ts), [strict web client](../apps/web/src/local-position-execution-client.ts), [execution UI](../apps/web/src/local-position-execution-panel.tsx), [migration](../infra/migrations/20260820000200_create_local_position_execution.sql) | [T-UNIT/T-API/T-SEC](../tests/p05-local-position-execution-api.test.ts), [T-API/T-RBAC](../tests/p05-local-position-execution-http-api.test.ts), [T-REC](../tests/p05-local-position-recovery.test.ts), [T-MIG/T-REC](../tests/integration/postgres-local-position-execution.integration.ts), [T-CHAIN](../tests/integration/anvil-local-position-execution.integration.ts), [T-UI/T-VIS](../tests/e2e/p05-07-local-position-execution.spec.ts) | [P05-07](../artifacts/acceptance/P05-07/manifest.json); local-fixture-verified; chainId 31337 and TestOnlyPositionManagerV2 only; BSC/testnet/production execution closed |
| POS-03 | `implemented-assumed` | [local Position Registry](../packages/chain-registry/src/local-position-execution.ts), [snapshot/plan and integer accounting](../packages/domain/src/local-position-execution.ts), [API service](../apps/api/src/local-position-executions.ts), [PostgreSQL proceeds/pricing ledger](../apps/api/src/postgres-local-position-execution-store.ts), [Signer authorizer](../apps/signer/src/postgres-local-position-plan-authorizer.ts), [Worker recovery](../apps/worker/src/postgres-local-position-recovery.ts), [strict web client](../apps/web/src/local-position-execution-client.ts), [execution UI](../apps/web/src/local-position-execution-panel.tsx), [TestOnlyPositionManagerV2](../contracts/src/TestOnlyPositionManagerV2.sol) | [T-UNIT/T-SEC](../tests/p05-local-position-contract.test.ts), [T-API/T-RBAC](../tests/p05-local-position-execution-http-api.test.ts), [T-REC](../tests/p05-local-position-recovery.test.ts), [T-MIG/T-REC](../tests/integration/postgres-local-position-execution.integration.ts), [T-CHAIN](../tests/integration/anvil-local-position-execution.integration.ts), [T-UI/T-VIS](../tests/e2e/p05-07-local-position-execution.spec.ts) | [P05-07](../artifacts/acceptance/P05-07/manifest.json); local-fixture-verified; ordered decrease -> collect -> optional burn; BSC/testnet/production execution closed |
...
| HELPER-02 | `implemented-assumed` | [deployment domain and CREATE plan](../packages/domain/src/helper-deployment.ts), [local Registry and fixed bytecode template](../packages/chain-registry/src/helper-deployment.ts), [API service](../apps/api/src/helper-deployments.ts), [PostgreSQL operation store](../apps/api/src/postgres-helper-deployment-store.ts), [Signer authorizer](../apps/signer/src/postgres-helper-deployment-plan-authorizer.ts), [Worker recovery](../apps/worker/src/postgres-helper-deployment-recovery.ts), [strict web client](../apps/web/src/helper-deployment-client.ts), [Helper deployment UI](../apps/web/src/helper-deployment-panel.tsx), [migration](../infra/migrations/20260819000400_create_helper_deployment_operations.sql) | [T-UNIT/T-API/T-SEC](../tests/p05-helper-deployment-api.test.ts), [T-API/T-RBAC](../tests/p05-helper-deployment-http-api.test.ts), [T-SEC](../tests/p05-helper-deployment-signer.test.ts), [T-REC](../tests/p05-helper-deployment-recovery.test.ts), [T-MIG/T-REC](../tests/integration/postgres-helper-deployment-recovery.integration.ts), [T-CHAIN](../tests/integration/anvil-helper-deployment.integration.ts), [T-UI](../tests/helper-deployment-client.test.ts), [T-UI/T-VIS](../tests/e2e/p05-05-helper-deployment.spec.ts) | [P05-05](../artifacts/acceptance/P05-05/manifest.json); local-fixture-verified; chainId 31337 and synthetic assets only; testnet/production closed |
| HELPER-03 | `implemented-assumed` | [WalletHelperV2](../contracts/src/WalletHelperV2.sol), [upgrade Registry and frozen artifact](../packages/chain-registry/src/local-helper-upgrade.ts), [snapshot/plan domain](../packages/domain/src/local-helper-upgrade.ts), [API service](../apps/api/src/local-helper-upgrades.ts), [PostgreSQL operation store](../apps/api/src/postgres-local-helper-upgrade-store.ts), [Signer authorizer](../apps/signer/src/postgres-local-helper-upgrade-plan-authorizer.ts), [Worker recovery and atomic CAS](../apps/worker/src/postgres-local-helper-upgrade-recovery.ts), [strict web client](../apps/web/src/local-helper-upgrade-client.ts), [upgrade UI](../apps/web/src/local-helper-upgrade-panel.tsx), [migration](../infra/migrations/20260821000100_create_local_helper_upgrade.sql) | [T-UNIT/T-API/T-SEC](../tests/p05-local-helper-upgrade-contract.test.ts), [T-API/T-RBAC](../tests/p05-local-helper-upgrade-http-api.test.ts), [T-SEC](../tests/p05-local-helper-upgrade-signer.test.ts), [T-REC](../tests/p05-local-helper-upgrade-recovery.test.ts), [T-MIG/T-REC](../tests/integration/postgres-local-helper-upgrade.integration.ts), [T-CHAIN](../tests/integration/anvil-local-helper-upgrade.integration.ts), [T-UI](../tests/p05-local-helper-upgrade-client.test.ts), [T-UI/T-VIS](../tests/e2e/p05-09-local-helper-upgrade.spec.ts) | [P05-09](../artifacts/acceptance/P05-09/manifest.json); local-fixture-verified; deploy-new only on chainId 31337; atomic liquidity and BSC/testnet/production execution closed |
...
| 当前产品实现 | 72 | P01 的 18 项、P02 的 23 项、P03 的 8 项、P04 的 12 项和 P05 的 11 项完成阶段实现；P05 为 11 implemented-assumed / 1 planned |
| `implemented-assumed` | 72 | 目标对照或 live 证据仍不完整 |
| `parity-verified` | 0 | 不由 accepted work item 自动提升 |
| `released` | 0 | 尚无 staging、监控和回滚完整证明 |
| 其余 `planned` | 124 | P02/P03/P04 已无 planned；P05 仍有 1 项 planned |
